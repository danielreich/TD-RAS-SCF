...
!! phi_rangang1(#1) and phi_rangang_2(#1) gives the global function basis set of the fedvr+angle+angle contribution to the two body integrals.
  ...
  integer :: l_itp, m_itp, l_rtp, m_rtp !! loops
  real (kind=k1) :: auxiliar_zz
  real (kind=k1) :: auxiliar_r_begin, auxiliar_r_end,auxiliar_r_inner
  integer :: auxiliar_inner_elements, auxiliar_nb_sub,auxiliar_elements
  real (kind=k1) :: length_rtp,length_itp
...
if(laser%tdornot) then
    ...
    If (auxiliar_io_sdtq.ne.system%io_sdtq) then
       write(*,*) 'ERROR: RAS SCHEME DIFFERS ON THE EXCITATION SCHEME'
       stop
    End If
    ...
    read(ifile,*) auxiliar_r_begin, auxiliar_r_inner, auxiliar_inner_elements, auxiliar_r_end, auxiliar_elements, auxiliar_nb_sub

!! CHECK THE FEDVR OF THE INNER REGION

    if (abs(auxiliar_r_begin-fedvr3d%r_begin).gt.1.0d-4) then
       write(*,*) 'ERROR: NOT THE SAME r_begin'
       write(*,*)
       stop
    End if

    if (abs(auxiliar_r_inner-fedvr3d%r_inner).gt.1.0d-4) then
       write(*,*) 'ERROR: NOT THE SAME r_end'
       write(*,*)
       stop
    End if

    if (auxiliar_inner_elements.ne.fedvr3d%number_of_element_inner) then
       write(*,*) 'ERROR: FEDVRs IN THE INNER REGION IS NOT THE SAME'
       write(*,*)
       stop
    End if

!! CHECK THAT THE IMAGINARY TIME PROPAGATION FEDVR IS COMPATIBLE WITH THE REAL TIME PROPAGATION

    If (fedvr3d%number_of_element.eq.auxiliar_inner_elements) then       
       continue       
    Else       
       length_rtp=(fedvr3d%r_end-fedvr3d%r_inner)/dble(fedvr3d%number_of_element-fedvr3d%number_of_element_inner)
       length_itp= (auxiliar_r_end-auxiliar_r_inner)/(auxiliar_elements-auxiliar_inner_elements)

       if (abs(length_rtp-length_itp).gt.1d-4) then !! THEY ARE NOT THE SAME
          write(*,*)
          write(*,*) 'ERROR: THE FEDVRs ARE NOT COMPATIBLE'
          write(*,*) 'THE LENGTH OF THE ELEMENTS IS NOT THE SAME IN THE'
          write(*,*) 'FEDVRs'
          write(*,*)
          stop
       end if
...
!! CHECK THE NUMBER OF NODES
    ...
    read(ifile,*) auxiliar_l_max,auxiliar_m_max
...
!! READING THE COEFFICIENTS OF THE SLATER DETERMINANTS
...
!! READING THE ORBITALS
    ...
    Do idicp=1,6  !! read the empty spaces and titles
       read(ifile,*) auxiliar_char
    End Do
    ...
    Do idicp=1, system%nptot
       ...
       Do l_itp= 0,auxiliar_l_max
          Do m_itp= -min(auxiliar_m_max,l_itp), min(auxiliar_m_max,l_itp) !! Run in the angular functions for ITP
             print*, 'ITP', l_itp, m_itp
             counter=0
             Do l_rtp=0,fedvr3d%l_max
                Do m_rtp= -min(fedvr3d%m_max,l_rtp), min(fedvr3d%m_max,l_rtp) !! Run in the angular functions for RTP
                   counter=counter+1
                   If (l_itp.eq.l_rtp.and.m_itp.eq.m_rtp) exit !! We have found the place to copy the ITP
                End Do
                If (l_itp.eq.l_rtp.and.m_itp.eq.m_rtp) exit !! We have found the place to copy the ITP
             End Do
             

             Do jdicp=1,(auxiliar_nb_sub-1)*auxiliar_elements-1
                Read(ifile,*) phi0(jdicp+(counter-1)*size(fedvrx_global),idicp) 
             End Do

          End Do
       End Do

       Do jdicp=1,4  !! read the empty spaces and titles
          read(ifile,*) auxiliar_char
       End Do
       
    End Do !! Run in the orbitals
...
!! End of initialize the seed
   ...
   do idicp =1, n_string_ab
     acoff(idicp) = dcmplx(sqrt( 1.0d0/dble(n_string_ab) ),0.0d0)
   enddo
...
!
   ...
   allocate(phi_state(1:fedvr3d%nb_r*fedvr3d%nb_angle))
     counter=0
!!$
Do n=1,system%np0!!system%nptot !! run in principal quantum number
...
deallocate(phi_state)
     ...
     Do counter=system%np0+1,size(phi(1,:))
...
phiuni=phiuni/sqrt(real(dot_product(phiuni,phiuni)))
...
!!===================================================================
!! The subroutine np0_lm sets the core to have well preserved angular 
!! momentum and magnetic quantum number.
!!===================================================================

Subroutine np0_lm(orbital,num_np0,lmax,mmax)
implicit none
!!INPUT
complex(kind=k2), allocatable,intent(inout) :: orbital(:,:) !! orbitals, where the #1 is the basis and #2 is the number of the orbital
integer, intent(in) :: num_np0 !! number of orbitals in the core
integer, intent(in) :: lmax, mmax !! maximum angular momentum and magnetic quantum number.

!!OUTPUT
!the orbitals

!!WORKSPACE
complex(kind=k2), allocatable :: orbital_aux(:) !! orbitals, where the #1 is the basis and #2 is the number of the orbital (limit to the core)
integer:: ii, jj, kk,nn !! index of the loops
integer :: counter, counter2
integer :: r_size !! number of total nodes of the fedvr basis

If (num_np0.eq.0) return !! if there is nothing in the core

!! First, check the number of angular functions

counter=0

Do ii=0, lmax,1
   Do jj=-min(ii,mmax), min(ii,mmax),1
      counter=counter+1      
   End Do
End Do

!! Second, we calculate the number of nodes in the FEDVR

r_size=size(orbital(:,1))/counter

!! Next, allocate the auxiliar orbitals.

allocate(orbital_aux(1:r_size*counter))
orbital_aux=zzero !! initialize them

counter=0 !! counter the number of orbitals in the core

Do nn=1, num_np0 !! Running in the main quantum number of the configuration
   counter2=0

   Do ii=0,min(nn-1,lmax),1 !! Running in l
      Do jj=-min(ii,mmax),min(ii,mmax),1 !! Running in m
         counter2=counter2+1 !! Angular function
         counter=counter + 1 !! Number of orbitals

         Do kk=1,r_size
            orbital_aux(r_size*(counter2-1)+kk)= orbital(r_size*(counter2-1)+kk,counter) !! store the nonzero values
         End Do

         orbital(:,counter)=zzero !! initialize the orbital
         orbital(:,counter)=orbital_aux(:) !! store the nonzero values
         orbital_aux=zzero
         if (counter.eq.num_np0) exit
      End Do
         if (counter.eq.num_np0) exit
   End Do
         if (counter.eq.num_np0) exit
...
deallocate(orbital_aux)

End Subroutine np0_lm
