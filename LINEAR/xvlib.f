...
C
C   xvxczza (x,v,w,dim)
C       Definition: x*dconjg(v(i)) + w(i) = w(i)
C       Dimensions: v(dim),w(dim),x
C
C   xvxxdzr (x,v,w,dim)
...
C ----------------------------------------------------------------------
C Library subroutine xvxczza
C
C Multiplication of a complex scalar with the complex conjugate of a
C complex vector, where the result is added to a further vector
C     x*dconjg(v(i)) +w(i) = w(i)
C
C NB Input w vector is overwritten on output
C-----------------------------------------------------------------------

      subroutine xvxczza (x,v,w,dim)
...
C-----------------------------------------------------------------------
C Library subroutine qvtxdz
C
C Multiplication of a real quadratic matrix with a complex vector
C     a(j,i)*v(j)=w(i)
C-----------------------------------------------------------------------

      subroutine qvtxdz (a,v,w,dim)
...
C-----------------------------------------------------------------------
      ...
      subroutine qvtxdz1 (a,v,w,phdim,dim)
...
C-----------------------------------------------------------------------
      ...
      do i = 1,dim
         w(i) = dcmplx(a(i,1,1),a(i,1,2))*v(1)
      enddo
      do j = 2,dim
         do i = 1,dim
            w(i) = w(i)+dcmplx(a(i,j,1),a(i,j,2))*v(j)
         enddo
      enddo
