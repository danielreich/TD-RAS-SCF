...
C        Dimensions: mat(dim1,dim2)
C
C    zeromxs (mat,dim1,dim2)
C        Definition: mat(i,j) = 0.0
C        Dimensions: mat(dim1,dim2)
C
...
C
C    cpvxidz (v,w,dim)
C        Definition: w(i) = dcmplx(0.d0,v(i))
C        Dimensions: v(dim),w(dim)
C
C    cpvxzdz (vr,vi,w,dim)
C        Definition: w(i) = dcmplx(vr(i),vi(i))
C        Dimensions: v(dim),w(dim)
C
C    cpvxzd (v,w,dim)
C        Definition: w(i) = v(i)
...
C-----------------------------------------------------------------------
      ...
      subroutine zeromxs(mat,dim1,dim2)
...
c-----------------------------------------------------------------------
c Library subroutine unitqxs
c
C sets up a unit matrix
C     mat(i,j) = 0, if i.ne.j
C     mat(i,j) = 1, if i.eq.j
c-----------------------------------------------------------------------
      subroutine unitqxs(mat,dim)
...
C ----------------------------------------------------------------------
c Library subroutine cpvidz
c
C copies a real vector to the imaginary part of a  complex vector
C     w(i) = v(i)
c-----------------------------------------------------------------------

      subroutine cpvidz (v,w,dim)
...
C ----------------------------------------------------------------------
c Library subroutine cpvzdz
c
C copies two real vectors to the real and imaginary part of a complex vector
C     w(i) = dcmplx(vr(i),vi(i))
c-----------------------------------------------------------------------

      subroutine cpvzdz (vr,vi,w,dim)

      integer dim,i
      real*8  vr(dim),vi(dim)
      complex*16  w(dim)

      do i = 1,dim
         w(i) = dcmplx(vr(i),vi(i))
      enddo
...
c
C initialises a complex vector
C    vec(i)=(1.0d0,0.0d0)
c-----------------------------------------------------------------------
...
c
C initialises an integer vector
C    vec(i)=1
c-----------------------------------------------------------------------
...
c
C initialises a logical vector (with .true.)
C    vec(i)=.true.
c-----------------------------------------------------------------------
...
c
C initialises a complex matrix
C    a(j,i)=(1.0d0,0.0d0)
c-----------------------------------------------------------------------
...
C vectors are not completely stored, but managed by index arrays
C     w(i) = v(i)
C
C-----------------------------------------------------------------------
